...
st.markdown("""
    <div style='background-color:#0e1117; padding:20px; border-radius:10px; border-left: 8px solid #00ffcc; margin-bottom:20px;'>
        <h1 style='margin:0; color:white;'>🚀 Infinity 4-Hour SMC Master Trading Hub</h1>
...
if user_key == PREMIUM_KEYS["4H_ACCESS"]:
    ...
    if st.button("🔄 ડેશબોર્ડ ડેટા રિફ્રેશ કરો"):
        with st.spinner("Fetching 4H SMC data bridge blocks..."):
            try:
                # ૧. 4H Live Touch Scanner
                if selected_scanner == "📈 4-Hour Live Touch Scanner":
                    ...
                    if response.status_code == 200:
                        ...
                        if data:
                            ...
                            df.columns = ["Stock", "Current Price", "OB High (Demand)", "OB Status", "OB Date/Time"]
                            st.success("🎯 Bullish Order Block Touch Scanner Active!")
                            st.table(df[["Stock", "Current Price", "OB High (Demand)", "OB Status", "OB Date/Time"]])
                        else: st.info("📊 ક્લાઉડ સ્ટોરેજમાં કોઈ ડેટા મળ્યો નથી.")
                
                # ૨. 4H Zone + 15M Volumetric Cross
                    ...
                    if response.status_code == 200:
                        ...
                        if data:
                            df = pd.DataFrame(data)
                            df.columns = ["Stock", "Current Price", "15M RSI", "OB Status", "Zone Origin Time"]
                            st.success("🔥 Infinity SMC Sharp Volumetric Reversal Active!")
                            st.table(df[["Stock", "Current Price", "15M RSI", "OB Status", "Zone Origin Time"]])
                        else: st.info("📊 ક્લાઉડ સ્ટોરેજમાં કોઈ ડેટા મળ્યો નથી.")
